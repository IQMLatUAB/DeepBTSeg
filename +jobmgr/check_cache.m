...
% function
% fprintf('check_cache 20\n');
files = find_file_dependencies(c.filename, {c.filename});
% fprintf('check_cache 22\n');
% Find the modification dates for each dependency
% 
%     function date = find_modification_date(file)
%         for idx_file = 1:length(file)
%             fprintf('find_modification_date front\n');
%             file_struct = dir(file{idx_file});
%             date{idx_file} = file_struct.date;
%             fprintf('find_modification_date last\n');
%         end
%     end
    function date = find_modification_date(file)
        for idx_file = 1:length(file)
%             fprintf('find_modification_date front\n');
%             file_struct = dir(file{idx_file});
            date{idx_file} = datetime;
%             fprintf('find_modification_date last\n');
        end
    end

% fprintf('check_cache 31\n');
dates = find_modification_date(files);
% fprintf('check_cache 33\n');
% Ensure the cache directory exists
[~,~,~] = mkdir(c.cache_dir);
[~,~,~] = mkdir(fullfile(c.cache_root, 'datefiles'));

%renew the data_file to prevent clearing cache while submit different
%jobs. by ZI MIN Feb/09/2021
for i = 1:numel(files)
    file = files{i};
    date = dates{i};
    ...
    function cache_ok = do_check_cache
        ...
        for i = 1:numel(files)
            ...
            try
                ...
                file_ok = strcmp(char(saved_date.date), char(date));
